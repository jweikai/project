...
public class AnchorTag extends AbstractClosingTag {
	...
	public int doStartTag() throws JspException {
		// 当前登录用户
		UserVO user = (UserVO) pageContext.getSession().getAttribute(Const.LOGIN_USER_SESSION_KEY);
		// 当前准备显示的链接对应的权限URL
		isAccess = user.hasAuthorityByUrl(action);		
		if ( !isAccess && "single-word".equals(super.cssClass) ) {
			href = "javascript:;";
		}else href = action.split("_")[1]; 
		action = null;
